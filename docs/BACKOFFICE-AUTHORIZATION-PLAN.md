...
상태: 구현 전 설계·작업 준비 완료. 이번 담당 범위는 **권한 생성 기반과 API 적용 방법 설계**다. 실행 코드와 DB DDL은 후속 작업에서 변경한다.

**담당 범위 확정:** 역할·권한 코드/매핑/seed, 권한 판단에 필요한 기관·배정·보호자 연결·열람 승인 데이터 모델, 공통 인가 정책과 Spring 연동 기반, API 개발자용 적용 가이드 및 권한 모듈 테스트를 준비한다. **개별 API 생성과 API별 어노테이션 부착·쿼리/DTO 변경은 API 담당자가 수행한다.** 아래 업무 흐름과 공개 정책은 권한 계약을 설명하기 위한 참고이며 업무 기능 전체의 구현을 이슈 #135에 포함하지 않는다.
...
사용자 결정: 이전 업무일지를 보려면 승인이 필요하다. 권한 담당은 승인 상태·범위·만료·회수 모델과 유효 승인 판정 기능을 제공한다. 요청/승인/거절/회수 API와 화면은 API 담당 범위다. 아래는 양쪽이 공유할 동작 계약이다.
...
`@EnableMethodSecurity`와 공통 정책 bean/재사용 어노테이션은 권한 담당이 준비한다. API 담당자가 자기 `@UseCase` Spring bean의 public 메서드에 `@PreAuthorize` 또는 제공된 어노테이션을 부착한다. Controller만 검사하면 다른 호출 경로를 놓칠 수 있어 UseCase를 주요 경계로 권장한다. 아래 코드는 적용 가이드 예시이며 실제 API에 부착하거나 현재 프로젝트에서 컴파일되는 구현은 아니다.
...
## 9. 권한 기반 구현 순서와 API 담당자 인계

이슈 #135는 아래 권한 산출물까지만 담당한다. URL 설계와 개별 Controller/UseCase 생성은 API 담당자가 결정한다.

| 단계 | 권한 담당 산출물 | 완료 조건 |
|---|---|---|
| 1 | 역할·권한 카탈로그와 역할 매핑, seed | 사진의 행동 권한과 사용자 확정 정책을 코드/DB로 표현 |
| 2 | 권한용 DB/migration, 기관·배정·연결·승인 모델/조회 계약 | 기관 경계·활성 관계·승인 범위를 판별하고 기존 데이터 보존 |
| 3 | BackofficeAccessPolicy, actor/scope 모델, 권한 조회·회수 반영 | 클라이언트 ID를 신뢰하지 않고 현재 상태로 허용/거부 결정 |
| 4 | Method Security 설정과 재사용 가능한 적용 수단 | Spring proxy를 통한 샘플 bean에서 허용/거부 검증 |
| 5 | 상세/목록/쓰기/승인 이력별 적용 가이드 | API 담당자가 권한·대상 정보·조회 scope를 연결할 수 있음 |
| 6 | 권한 모듈 테스트와 분리 계약 | 매트릭스·교차 기관·관계 종료·승인 만료 테스트 통과 |

API 담당자에게 전달할 순서:

1. 기능에 맞는 권한 코드를 선택한다. actor는 인증 principal에서 얻고 요청의 역할/memberId를 신뢰하지 않는다.
2. UseCase 진입점에 제공된 인가 어노테이션 또는 정책 bean 호출을 연결한다. 기관 ID와 대상 ID를 정책에 전달한다.
3. 목록/검색/count/export 쿼리에 권한 모듈의 scope를 적용한다. 어노테이션 하나로 행 필터링까지 자동 처리된다고 가정하지 않는다.
4. 변경 기능은 같은 트랜잭션 안에서 관계/승인 상태를 재검증한다. 관계와 승인 변경 API는 공통 모델의 제약과 회수 계약을 따른다.
5. 응답 DTO에서 허용된 필드만 선택한다. 원문 비공개와 보호자 공개 제한은 API 담당자가 실제 직렬화 결과까지 검증한다.
6. 권한 담당이 제공하는 허용/거부 사례를 해당 API의 통합 테스트에 연결한다.

기관 초대·승인, 대상자 등록/조회, 배정 변경, 보호자 연결, 업무일지/조치, 이전 일지 승인, 보호자 공개 API의 생성과 권한 부착은 모두 API 담당 범위다. 실제 엔드포인트 테스트도 해당 API 작업에서 수행한다. 권한 기반의 완료와 실제 API 보호 완료를 구분한다.

기존 JWT 만료 처리 문제는 인증 담당자와 공유할 선행 의존성으로 유지한다. 이슈 #135에서 기존 로그인/재발급 API 개편이나 신규 인증 API 생성을 자동으로 포함하지 않는다. 권한 모듈은 검증된 principal과 활성 상태 조회 계약을 입력으로 받는다.
...
## 11. 담당 범위별 수용 기준과 테스트 계획

권한 담당의 완료 조건:

- 역할·권한 코드/매핑/seed와 권한 모델이 사진의 정책 및 이전 일지 승인 정책을 표현한다.
- 정책/Repository 테스트에서 기관 A/B, 담당자 A/B, 연결/미연결 보호자, ACTIVE/PENDING/SUSPENDED 상태별 허용/거부를 검증한다.
- Spring proxy를 통하는 테스트용 bean으로 Method Security와 재사용 어노테이션을 검증한다. 테스트용 예제를 실제 업무 API 생성으로 확장하지 않는다.
- 배정/연결 종료, 역할 회수, 기관 비활성화 후 다음 권한 판정이 거부된다. 승인 전/거절/범위 밖/만료/회수/재배정 시 이전 일지 권한을 허용하지 않는다.
- scope 계약이 상세뿐 아니라 목록/count/export에 적용 가능하고, 공개 필드 및 변경 시 재검증 책임이 적용 가이드에 명시되어 있다.
- MySQL 기반 테스트로 권한 관련 FK/활성 UNIQUE/승인 범위 및 migration을 검증한다. 담당 범위의 기존 테스트 회귀를 확인한다.
- 권한 모듈의 DB/정책/어노테이션/인계 문서가 준비되면 이슈 #135의 기반 작업 완료로 판단한다. 개별 API 적용 여부를 권한 기반 완료와 혼동하지 않는다.

API·인증 담당자에게 인계할 검증 조건:

- 실제 URL/UseCase에 권한 검사가 연결됐는지, 목록/상세/집계/count/export에 scope가 적용됐는지 검사한다.
- API 응답·다운로드에 원문/S3 key/URL이 없고 보호자에게 내부 사유·근거가 노출되지 않는지 검증한다.
- 로그인/재발급 및 초대 토큰 검증, 인증 영역 분리, 401/403/404 응답, 실제 토큰 재사용 시 권한 회수를 검증한다.
- 배정/승인/업무 쓰기 API의 동시성·트랜잭션·공개 DTO와 업무 기능 자체를 검증한다.

이번 변경은 범위 정리 문서만 수정하므로 애플리케이션 테스트는 실행하지 않는다.
