...
/**
 * 대상자 상세(기록 상세 화면). 상태 코드가 X인 대상자도 열 수 있도록 대상자 기준으로 조회하고,
...
public class GetRecipientUseCase {
    ...
    private RecipientListResponse.Manager currentManager(CareRecipient recipient) {
        return assignmentRepository.findCurrentByRecipient(recipient).stream()
                ...
                .map(worker -> {
                    ...
                    return new RecipientListResponse.Manager(account.getAccountUuid(), account.getName());
                })
                .orElse(null);
    }
}
